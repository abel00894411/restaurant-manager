	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http, AuthenticationManager authenticationManager)
		...
		return http.csrf(config -> config.disable())
				...
				.addFilterAfter(authFilter, AuthLoginFilter.class).authorizeHttpRequests(request -> {
					request.requestMatchers("/login").permitAll().requestMatchers("/prueba").permitAll()
							.requestMatchers("/obtenerTodosPrueba").permitAll().requestMatchers("/app/**").permitAll()
							.requestMatchers("/topic/**").permitAll().requestMatchers("/connect/**").permitAll()
							.requestMatchers("/prueba-empleados-activos").permitAll().requestMatchers("/usuarios/**")
							.hasAuthority("ADMINISTRADOR").requestMatchers(HttpMethod.GET, "/usuario/{id:\\d+}")
							.hasAnyRole("ADMINISTRADOR", "MESERO", "COCINERO")
							.requestMatchers(HttpMethod.PUT, "/usuario/{id:\\d+}").hasRole("ADMINISTRADOR")
							.requestMatchers(HttpMethod.PATCH, "/usuario/{id:\\d+}")
							.hasAnyRole("ADMINISTRADOR", "MESERO", "COCINERO")
							.requestMatchers(HttpMethod.DELETE, "/usuario/{id:\\d+}").hasRole("ADMINISTRADOR")
							.requestMatchers(HttpMethod.GET, "/menu").permitAll().requestMatchers("/menu/**")
							.hasRole("ADMINISTRADOR").requestMatchers("/ordenes/**").hasRole("ADMINISTRADOR")
							.requestMatchers("/orden/**").hasRole("ADMINISTRADOR").requestMatchers("/facturas")
							.permitAll().requestMatchers(HttpMethod.DELETE, "/factura/**").hasRole("ADMINISTRADOR")
